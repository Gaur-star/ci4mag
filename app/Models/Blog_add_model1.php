...
class Blog_add_model extends Model
{
  ...
  ////////we will do RND from here
    ...
    {
        $builder = $this->db->table("menu_list");
        $res = $builder->insert(array("menu_id" => $data['id'],"label"=>$data['label'] ,"custom_link" => $data['link']));
    }
    ...
    function preview($id,$date,$data)
    {
      ...
      //  print_r($d[0]['date_']);die;
       ...
       // print_r($perma);die;
        ...
        //   $d = $row['date_'];
          $d_create = date_create($d[0]['date_']);
        //  print_r($d_create);die;
          $date = date_format($d_create,$perma['linkformat']);
        //  print_r($data['content']);die;
        $sql="INSERT INTO `postPreview`(`post_id`, `title`, `content`, `seo_url`,`date_`) VALUES ('{$data['post_id']}','{$data['title']}','{$data['content']}','{$data['seo_url']}','{$date}')";
        $this->db->query($sql);
    ...
    function post_add_preview($data)
    {
       // echo $date."ssssss".$content;die;
        $db = db_connect();
      ...
      //  $sql = "UPDATE `posts` SET `visibility`='{$data['visibility']}',`title`='{$data['title']}',`seo_url`='{$data['seo_url']}',`content` = '{$data['content']}',`date_` = '{$data['date']}' WHERE `id` = '{$res[0]['id']}'";  
        $sql = "UPDATE `posts` SET `visibility`='{$data['visibility']}', `title`='{$data['title']}', `seo_url`='{$data['seo_url']}', `content` = '{$data['content']}', `date_` = '{$data['date']}', `image` = '{$data['image']}' WHERE `id` = '{$res[0]['id']}'";   
        $this->db->query($sql);

        // $sql1 = "INSERT INTO `postPreview` (`post_id`, `title`, `content`, `seo_url`, `date_`) VALUES ('{$res[0]['id']}','{$data['title']}','{$data['content']}','{$data['seo_url']}','{$data['date']}')";   
        // $this->db->query($sql1);
    ...
    function post_add_the_publish($data,$cat,$tags,$img_result)
    {
        $db = db_connect();
        $sql = "SELECT `id` FROM `posts` ORDER BY `id` DESC LIMIT 1";
        $res = $this->db->query($sql)->getResultArray();
        $builder = $this->db->table('posts');
        $builder->where('id',$res[0]['id']);
        $builder->update($data);

        $sql2 = "DELETE FROM `post_categories` WHERE `post_id` = ".$res[0]['id']."";
        $this->db->query($sql2);
        $builder = $this->db->table('post_categories');
        if (!empty($cat)) {
            foreach ($cat as $c) {
        ...
        $sql3 = "DELETE FROM `post_keywords` WHERE `post_id` = ".$res[0]['id']."";
        $this->db->query($sql3);
        $b = $this->db->table('post_keywords');
        if ($tags) {
            foreach ($tags as $t) {
    ...
    public function checkeditUrl($url)
    {
        ...
        if(count($results) > 1){
            ...
            $sql1 = "SELECT `id`,`seo_url` FROM `posts` WHERE `seo_url` = '".$url."'";
    ...
    public function new_insert_posts($data,$cat,$tag)
    {
        $sql3 = "SELECT `id`,`seo_url` FROM `posts` WHERE `seo_url` = '".$data['seo_url']."' ORDER BY `posts`.`id` DESC";
        $query3 = $this->db->query($sql3);
        $results = $query3->getRow();
        if($results){
            $data['seo_url'] = $results->seo_url.'-01';
        }else{
        ...
        $sql1 = "DELETE FROM `post_categories` WHERE `post_id` = ".$last_id."";
        $query1 = $this->db->query($sql1);
        foreach($cat as $val1){
            $builder=$this->db->table('post_categories');
            $builder->insert([
        ...
        $sql2 = "DELETE FROM `post_keywords` WHERE `post_id` = ".$last_id."";
        ...
        foreach($tag as $val2){
            ...
            $builder->insert([
                'post_id' => $last_id,
                'keyword' => $val2,
                'slug' => str_replace(' ','-',$val2),
            ]);
        }

        return $last_id;
    }
    public function new_update_posts($data,$cat,$tag,$id)
    {
        $sql = "SELECT `seo_url`,`date_` FROM `posts` WHERE `id` = ".$id."";
        $query = $this->db->query($sql);
        $results = $query->getRow();
        if(($results->seo_url != $data['seo_url']) && ($results->date_ != $data['date_'])){
            $builder=$this->db->table('pastUrl');
            $builder->insert([
                'post_id' => $id,
                'url' => $results->seo_url,
                'new_url' => $data['seo_url'],
                'old_date' => $results->date_,
            ]);
            $sql3 = "UPDATE `pastUrl` SET `new_url` = '".$data['seo_url']."' WHERE `pastUrl`.`post_id` = ".$id."";
            $query3 = $this->db->query($sql3);

            $builder=$this->db->table('posts');
            $builder->where('id',$id);
            $builder->update($data);
        }elseif(($results->seo_url != $data['seo_url']) && ($results->date_ == $data['date_'])){
            $builder=$this->db->table('pastUrl');
            $builder->insert([
                'post_id' => $id,
                'url' => $results->seo_url,
        ...
        $sql1 = "DELETE FROM `post_categories` WHERE `post_id` = ".$id."";
        $query1 = $this->db->query($sql1);
        foreach($cat as $val1){
            $builder=$this->db->table('post_categories');
            $builder->insert([
        ...
        $sql2 = "DELETE FROM `post_keywords` WHERE `post_id` = ".$id."";
        $query2 = $this->db->query($sql2);
        $tag = explode(',',$tag);
        foreach($tag as $val2){
            $builder=$this->db->table('post_keywords');
    ...
    public function new_auto_update($data,$cat,$tag,$id)
    {
    	$sql3 = "SELECT `id`,`seo_url` FROM `posts` WHERE `id` <> ".$id." AND `seo_url` = '".$data['seo_url']."' ORDER BY `posts`.`id` DESC";
        $query3 = $this->db->query($sql3);
        $results = $query3->getRow();
        if($results){
            $data['seo_url'] = $results->seo_url.'-01';
        ...
        $sql5 = "SELECT * FROM `permalink_list` WHERE status = 'active'";
        $query5 = $this->db->query($sql5);
        $linkformat = $query5->getRow()->linkformat;
        if($linkformat){
            $url = base_url().'/'.date($linkformat,strtotime($data['date_'])).'/'.$data['seo_url'];
        }else{
            $url = base_url().'/'.$data['seo_url'];
        }
        return $url;
    }
    public function new_preview_post($id)
    {
        $sql1 = "SELECT * FROM `permalink_list` WHERE status = 'active'";
        $query1 = $this->db->query($sql1);
        $linkformat = $query1->getRow()->linkformat;
        
        $sql = "SELECT `id`,`title`,`content`,`seo_url`,`date_` FROM `posts` WHERE `id` = ".$id."";
        $query = $this->db->query($sql);
        $results = $query->getRow();
        ...
        if($linkformat){
            $url = base_url().'/'.date($linkformat,strtotime($results->date_)).'/'.$results->seo_url.'/preview';
        }else{
            $url = base_url().'/'.$results->seo_url.'/preview';
        }
        return $url;
    }
