                        ...
                        <?php  if ($roleId) { ?>
                          ...
                          <option value="<?= $roleId ?>"><?= $u_firstname ?></option>
                            <?php foreach (get_author() as $author) { ?>
                              <option value="<?php echo $author['uid']; ?>"<?php echo isset($post_data['author']) ? "selected" : "" ?>><!-- &nbsp; --><?php echo $author['f_name'] . " " . $author['l_name']; ?>
                              </option>
                            <?php } ?>
    ...
    /////////////////////////////////
     ...
     $.ajax({
       ...
       data:{title:title,seo_url:seo_url, content:source, last_dreaft_id:last_dreaft_id,visibility:visibility,date_:date_,time_:time_,update_date:update_date,nofollow:nofollow,seo_url_no:seo_url_no,post_parent:post_parent,seo_url_text:seo_url_text,image:image,
        cat:cat_id,keyword_list:keyword_list,
        meta_tag:meta_tag,meta_desc:meta_desc,date_time:date_time},
       success:function(data)
       {
        draft_working = false;
        if(last_dreaft_id == 0) {
...
<script>
  $(document).ready(function(){
   ...
   // filebrowserUploadUrl: 'http://localserver.com/wp2ci/assets/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Files',
    filebrowserUploadUrl: '<?php echo base_url();?>/admin/home/upload_ck',
